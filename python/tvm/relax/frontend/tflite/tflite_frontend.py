...
class OperatorConverter:
    ...
    def _convert_reduce_bool(self, relax_op, op):
        """Convert TFLite REDUCE_ANY / REDUCE_ALL.

        TFLite REDUCE_ANY (logical OR) maps to max, REDUCE_ALL (logical AND)
        maps to min. Relax max/min are not defined for bool, so cast through
        int8 and cast the result back to bool.
        """
        from tflite.BuiltinOptions import BuiltinOptions
        from tflite.ReducerOptions import ReducerOptions
